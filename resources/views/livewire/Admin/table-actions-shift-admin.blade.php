    ...
    <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded-lg shadow-lg z-50 overflow-y-auto">
      ...
      <div class="modal-content py-4 text-left">
        ...
        <div class="flex justify-between items-center pb-3 border-b px-6">
          <p class="text-2xl font-base text-gray-700">Editing <span class="font-semibold">{{$shift->name}}</span></p>
          <div class="modal-close cursor-pointer z-50" onclick="toggleModal('edit{{$id}}')">
            ...
            <form action="{{route('admin.shift.update',$id)}}" method="POST" >
            @csrf
            @method('PUT')
                <div class="bg-white  pt-5 pb-4 sm:p-6 sm:pb-4 ">
                        ...
                        <div class="mb-4">
                            <label for="formName" class="block text-gray-600 text-sm font-semibold mb-2">Shift Name:</label>
                            <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-600 leading-tight focus:outline-none focus:shadow-outline" id="formName" name="name" value="{{$shift->name}}">
                            @error('name') <span class="text-red-500">{{ $message }}</span>@enderror
                        </div>
                        <div class="flex gap-4 flex-row">
                        <div class="flex-auto">
                            <label for="formEmail" class="block text-gray-600 text-sm font-semibold mb-2">Time In:</label>
                            <input type="time" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-600 leading-tight focus:outline-none focus:shadow-outline" id="formEmail" name="time_in" value="{{$shift->time_in}}">
                            @error('time_in') <span class="text-red-500">{{ $message }}</span>@enderror
                        </div>
                        <div class="flex-auto">
                            <label for="formPhone" class="block text-gray-600 text-sm font-semibold mb-2">Time Out:</label>
                            <input type="time" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-600 leading-tight focus:outline-none focus:shadow-outline" id="formPhone" name="time_out" value="{{$shift->time_out}}">
          ...
          <div class="z-50 relative p-3 mx-auto my-0 max-w-full">
              <div class="text-center flex flex-col overflow-hidden px-10 py-10 my-auto">
                  ...
                  <div class=" font-light text-gray-700 mb-8">
                      Do you really want to delete <span class="font-semibold">{{$shift->name}}</span> ? This process cannot be undone.
                  </div>
                  <div class="flex justify-center gap-2  ">
                      ...
                      <form action="{{route('admin.shift.destroy',$id)}}" method="POST">
                      @csrf
                      @method('DELETE')
