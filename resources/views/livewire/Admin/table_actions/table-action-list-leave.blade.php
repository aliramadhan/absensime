    ...
    <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded-lg shadow-lg z-50 overflow-y-auto">
      ...
      <div class="modal-content py-4 text-left">
        ...
        <div class="flex justify-between items-center pb-3 border-b px-6">
          <p class="text-2xl font-base text-gray-700">Editing <span class="font-semibold">{{$leave->name}}</span></p>
          <div class="modal-close cursor-pointer z-50" onclick="toggleModal('edit{{$id}}')">
            ...
            <form action="{{route('admin.leave.update',$id)}}" method="POST" >
            @csrf
            @method('PUT')
                <div class="bg-white  pt-5 pb-4 sm:p-6 sm:pb-4 ">
                        <div class="mb-4">
                            <label for="formName" class="block text-gray-600 text-sm font-semibold mb-2">Leave Name:</label>
                            <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-600 leading-tight focus:outline-none focus:shadow-outline" id="formName" name="name" value="{{$leave->name}}">
          ...
          <div class="z-50 relative p-3 mx-auto my-0 max-w-full">
              <div class="text-center flex flex-col overflow-hidden px-10 py-10 my-auto">
                  ...
                  <div class=" font-light text-gray-700 mb-8">
                      Do you really want to delete <span class="font-semibold">{{$leave->name}}</span> ? This process cannot be undone.
                  </div>
                  <div class="flex justify-center gap-2  ">
                      ...
                      <form method="POST" action="{{route('admin.leave.destroy',$id)}}">
                      @csrf
                      @method('delete')
